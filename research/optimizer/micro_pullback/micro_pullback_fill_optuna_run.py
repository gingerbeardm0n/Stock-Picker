"""
micro_pullback_fill_optuna_run.py — Micro-Pullback re-optimization under the
marketable-limit fill model (docs/SIM_FILL_MODEL_DESIGN.md step 4).

Same 14-param search space as micro_pullback_optuna_run.py plus a tunable
entry headroom (15 params), evaluated with multi mode (arm 10 / max 3) and
fill_model='marketable_limit'.

Diagnostic that motivated this: Trial 167 sealed-2025 PnL drops from
+$3,420 (perfect fills) to +$686 (realistic fills) with 21% of entries
missing — breakout-on-expanding-volume entries fight marketable limits
structurally, so the optimizer must feel that pressure.

Walk-forward protocol unchanged:
    TRAIN   2021-2023 | SELECT 2024 (plateau) | TEST 2025 SEALED (once)

Usage:
    python micro_pullback_fill_optuna_run.py --start 2021-01-01 --end 2023-12-31 --trials 200 --study mp_fill_v1
    python micro_pullback_fill_optuna_run.py --start 2024-01-01 --end 2024-12-31 --validate-trial N --study mp_fill_v1
"""
...
from optimizer.micro_pullback.micro_pullback_optuna_run import (
...
def _fill_config_from_trial(trial: optuna.Trial):
    ...
    return replace(
        cfg,
        fill_model='marketable_limit',
        entry_headroom_pct=trial.suggest_float('m_entry_headroom_pct', 0.05, 1.0),
    )
...
def _fill_config_from_params(params: dict):
    ...
    return replace(
        cfg,
        fill_model='marketable_limit',
        entry_headroom_pct=params['m_entry_headroom_pct'],
    )
...
def create_objective(start_date: str, end_date: str, account_size: float):
    def objective(trial: optuna.Trial) -> float:
        ...
        result = run_micro_pullback_date_range_multi(
...
def main():
    parser = argparse.ArgumentParser(description='Micro-Pullback optimizer (fill model ON)')
    parser.add_argument('--start', required=True)
    parser.add_argument('--end', required=True)
    parser.add_argument('--trials', type=int, default=200)
    parser.add_argument('--study', default='mp_fill_v1')
