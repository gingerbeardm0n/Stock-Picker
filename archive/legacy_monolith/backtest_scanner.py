...
def backtest_single_day(date, max_stocks=None, scan_time=None, progress_callback=None):
    ...
    with StockDataDB() as db:
        ...
        # BATCH QUERY 2: Hourly premarket bars (4am-8am) + minute bars (8am->scan time)
        report(3, f"Fetching premarket hourly bars (4am-8am) + minute bars (8am-{scan_hour:02d}:{scan_minute:02d})...", 40)
        hour_data_all = db.get_hour_bars(symbols, date, start_hour=4, end_hour=8)
        minute_start_hour = 8
        minute_end_hour = max(scan_hour + 1, minute_start_hour)
        minute_data_all = db.get_minute_bars(symbols, date, start_hour=minute_start_hour, end_hour=minute_end_hour)
        report(3, f"Hour bars loaded for {len(hour_data_all):,} symbols; minute bars loaded for {len(minute_data_all):,} symbols", 55)

        # BATCH QUERY 3: Historical avg volume at this same time of day (hybrid 4am-8am hour + 8am->time minute)
        report(4, f"Fetching historical same-time volume averages...", 60)
        avg_at_time = db.get_avg_volume_at_time_batch(
            symbols, date, scan_hour, scan_minute, lookback_days=20, include_premarket_hourly=True
        )
        ...
        for i, symbol in enumerate(symbols):
            ...
            try:
                ...
                # Premarket volume from hourly bars (4am-8am)
                hour_bars = hour_data_all.get(symbol, [])
                premarket_volume = sum(bar['volume'] for bar in hour_bars)

                # Market hours volume from minute bars (9:30am+)
