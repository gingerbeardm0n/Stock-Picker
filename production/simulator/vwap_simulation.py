...
class VwapSimulationRunner:
    ...
    def run_multi(self, max_armed: int = 10, max_concurrent: int = 3) -> dict:
        ...
        own reclaim signal while < `max_concurrent` positions open."""
        with StockDataDB() as db:
            candidates = db.find_gappers(
                self.trade_date,
                min_gap_pct=self.config.min_gap_pct,
    ...
    def _simulate_trades_multi(
        ...
        """Lockstep walk over all armed symbols' bars: per-symbol VWAP
        accumulator, concurrent entry cap, per-symbol exit tracking."""
        ...
        for c in armed:
            ...
            meta[sym] = {
                ...
                'acc': VwapAccumulator(), 'bars_so_far': [],
                'position': None,
            }
            ...
            if pos is not None:
                ...
                if open_count < max_concurrent:
                    ...
                    signal = evaluate_entry(
                        m['candidate'], m['bars_so_far'], vwap, self.config)
                    if signal:
                        entry_price = signal['entry_price']
                        stop_price = signal['stop_price']
                        m['position'] = {
                        ...
                        open_count += 1
                        continue
                if bar.get('_et') and bar['_et'].hour >= 12:
                    m['done'] = True
    ...
    def _close_multi(self, m: dict, pos: dict, bar: dict,
                     bar_idx: int, exit_signal: dict) -> VwapTrade:
        c = m['candidate']
        ...
        return VwapTrade(
            ...
            vwap_at_entry=pos.get('vwap', 0.0),
...
def run_vwap_date_range_multi(
    ...
    """Multi-candidate variant of run_vwap_date_range (live-parity behavior)."""
    with StockDataDB() as db:
        trading_days = db.get_trading_days(start_date, end_date)
